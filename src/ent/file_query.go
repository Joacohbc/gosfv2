...
func (fq *FileQuery) QuerySharedWith() *UserQuery {
	query := (&UserClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(file.Table, file.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, file.SharedWithTable, file.SharedWithPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(fq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (fq *FileQuery) QueryNotes() *NoteQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, file.NotesTable, file.NotesColumn),
		)
		fromU = sqlgraph.SetNeighbors(fq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (fq *FileQuery) WithSharedWith(opts ...func(*UserQuery)) *FileQuery {
	query := (&UserClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withSharedWith = query
	return fq
}
...
func (fq *FileQuery) WithNotes(opts ...func(*NoteQuery)) *FileQuery {
	query := (&NoteClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withSharedWith; query != nil {
		if err := fq.loadSharedWith(ctx, query, nodes,
			func(n *File) { n.Edges.SharedWith = []*User{} },
			func(n *File, e *User) { n.Edges.SharedWith = append(n.Edges.SharedWith, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := fq.withNotes; query != nil {
		if err := fq.loadNotes(ctx, query, nodes, nil,
			func(n *File, e *Note) { n.Edges.Notes = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadSharedWith(ctx context.Context, query *UserQuery, nodes []*File, init func(*File), assign func(*File, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint]*File)
	nids := make(map[uint]map[*File]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(file.SharedWithTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(file.SharedWithPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(file.SharedWithPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(file.SharedWithPrimaryKey[0]))
...
func (fq *FileQuery) loadNotes(ctx context.Context, query *NoteQuery, nodes []*File, init func(*File), assign func(*File, *Note)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*File)
	for i := range nodes {
		if nodes[i].note_files == nil {
			continue
		}
		fk := *nodes[i].note_files
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "note_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
