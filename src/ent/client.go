...
// QuerySharedWith queries the shared_with edge of a File.
func (c *FileClient) QuerySharedWith(f *File) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := f.ID
		step := sqlgraph.NewStep(
...
// QueryNotes queries the notes edge of a File.
func (c *FileClient) QueryNotes(f *File) *NoteQuery {
...
// QuerySharedWith queries the shared_with edge of a Note.
func (c *NoteClient) QuerySharedWith(n *Note) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := n.ID
		step := sqlgraph.NewStep(
...
// QuerySharedFiles queries the shared_files edge of a User.
func (c *UserClient) QuerySharedFiles(u *User) *FileQuery {
	query := (&FileClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
// QuerySharedNotes queries the shared_notes edge of a User.
func (c *UserClient) QuerySharedNotes(u *User) *NoteQuery {
	query := (&NoteClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		File, Note, User []ent.Hook
	}
	inters struct {
		File, Note, User []ent.Interceptor
	}
)
