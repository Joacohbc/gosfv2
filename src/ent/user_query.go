...
type UserQuery struct {
	...
	withFiles       *FileQuery
	withSharedFiles *FileQuery
	withSharedNotes *NoteQuery
	withNotes       *NoteQuery
...
// QuerySharedFiles chains the current query on the "shared_files" edge.
func (uq *UserQuery) QuerySharedFiles() *FileQuery {
	query := (&FileClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(file.Table, file.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, user.SharedFilesTable, user.SharedFilesPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QuerySharedNotes chains the current query on the "shared_notes" edge.
func (uq *UserQuery) QuerySharedNotes() *NoteQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, user.SharedNotesTable, user.SharedNotesPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSharedFiles: uq.withSharedFiles.Clone(),
		withSharedNotes: uq.withSharedNotes.Clone(),
...
// WithSharedFiles tells the query-builder to eager-load the nodes that are connected to
// the "shared_files" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSharedFiles(opts ...func(*FileQuery)) *UserQuery {
	query := (&FileClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSharedFiles = query
	return uq
}

// WithSharedNotes tells the query-builder to eager-load the nodes that are connected to
// the "shared_notes" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSharedNotes(opts ...func(*NoteQuery)) *UserQuery {
	query := (&NoteClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSharedNotes = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [4]bool{
			uq.withFiles != nil,
			uq.withSharedFiles != nil,
			uq.withSharedNotes != nil,
			uq.withNotes != nil,
		}
	)
	...
	if query := uq.withSharedFiles; query != nil {
		if err := uq.loadSharedFiles(ctx, query, nodes,
			func(n *User) { n.Edges.SharedFiles = []*File{} },
			func(n *User, e *File) { n.Edges.SharedFiles = append(n.Edges.SharedFiles, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withSharedNotes; query != nil {
		if err := uq.loadSharedNotes(ctx, query, nodes,
			func(n *User) { n.Edges.SharedNotes = []*Note{} },
			func(n *User, e *Note) { n.Edges.SharedNotes = append(n.Edges.SharedNotes, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadSharedFiles(ctx context.Context, query *FileQuery, nodes []*User, init func(*User), assign func(*User, *File)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SharedFilesTable)
		s.Join(joinT).On(s.C(file.FieldID), joinT.C(user.SharedFilesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.SharedFilesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SharedFilesPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "shared_files" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadSharedNotes(ctx context.Context, query *NoteQuery, nodes []*User, init func(*User), assign func(*User, *Note)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint]*User)
	nids := make(map[uint]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SharedNotesTable)
		s.Join(joinT).On(s.C(note.FieldID), joinT.C(user.SharedNotesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.SharedNotesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SharedNotesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "shared_notes" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
